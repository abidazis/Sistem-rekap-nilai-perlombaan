<div> <div class="bg-slate-900 p-6 rounded-lg shadow-lg mb-8 border-b-4 border-yellow-500 sticky top-0 z-40">
    ...
    @if (session()->has('message'))
        <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6 font-bold text-center animate-bounce shadow-md">
            {{ session('message') }}
        </div>
    @endif
        ...
        <form wire:submit.prevent="simpan">
                ...
                @foreach($struktur_penilaian as $kategori)
                    <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                        ...
                        <div class="bg-gradient-to-r from-blue-800 to-blue-600 px-6 py-4 flex justify-between items-center">
                            ...
                            <span class="bg-yellow-400 text-blue-900 font-bold text-xs px-3 py-1 rounded-full shadow-sm">
                                Bobot: {{ (float) $kategori->bobot_persen }}%
                            </span>
                        </div>
                                        ...
                                        <tr class="border-b border-gray-200 hover:bg-blue-50 transition-colors duration-200">
                                            ...
                                            <td class="py-4 px-4">
                                                <div class="flex flex-wrap items-center gap-2 md:gap-3">
                                                    @php
                                                        // Trik Jitu: Paksa tambahkan angka 0 di awal array otomatis!
                                                    ...
                                                    @if(count($opsi) > 0)
                                                        @foreach($opsi as $val)
                                                            @php
                                                                // LOGIKA WARNA KHUSUS UNTUK NILAI 0
                                                                $isZero = ($val == '0' || $val == 0);
            ...
            <div class="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur border-t border-gray-300 p-4 shadow-[0_-10px_20px_rgba(0,0,0,0.1)] z-50">
                <div class="container mx-auto flex justify-between items-center max-w-7xl">
                    ...
                    <button type="submit" 
                        class="w-full md:w-1/3 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white font-black text-lg py-4 px-10 rounded-xl shadow-xl transform transition hover:scale-105 border-b-4 border-green-800 focus:outline-none focus:ring-4 focus:ring-green-300"
                    >
                        💾 SIMPAN NILAI
                    </button>
                </div>
            ...
            <div class="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur border-t border-gray-300 p-4 shadow-[0_-10px_20px_rgba(0,0,0,0.1)] z-50">
                <div class="container mx-auto flex justify-between items-center max-w-7xl">
                    <div class="text-sm text-gray-600 hidden md:flex flex-col">
                        <span class="font-bold text-blue-900 text-lg uppercase">Pastikan semua nilai terisi!</span>
                        <span>Klik kotak nilai untuk memilih. Form otomatis menolak jika ada yang kosong.</span>
                    </div>
                    <button type="submit" 
                        class="w-full md:w-1/3 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white font-black text-lg py-4 px-10 rounded-xl shadow-xl transform transition hover:scale-105 border-b-4 border-green-800 focus:outline-none focus:ring-4 focus:ring-green-300"
                    >
